---
title: "Einfuehrung in R"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{einfuehrung}
...
### Was wir in der ersten Woche machen werden
...
1. Twitter Daten sammeln lernen
2. Verschiedene Auswertungsmöglichkeiten für Twitter-Daten kennenlernen
    - deskriptive Analyse
    - Visualisierungsmöglichkeiten: Wordclouds & Netzwerke
    - Sentiment-Analyse
    - Boterkennung
...
#so können wir Daten aus eineer Datei laden
data("twitch_data")

```


Man kann auch *inline-code* verwenden, das sieht dann so aus: Der Datensatz umfasst `r dim(twitch_data)[1]` Beobachtungen.

Dies erlaubt es, ein Dokument mit relativen Bezügen zu erstellen, ähnlich einer Excel-Tabelle! Wenn sich also bspw. die Datengrundlage ändert, müssen wir nicht mühsam alle Bezüge im Text anpassen. Noch fortgeschrittener lassen sich sogenannte _parameterisierte_ Reports erzeugen, bei dem man dem Befehl zur Erzeugung des Dokuments einen oder mehrere Parameter beifügt. Auf diese Weise lassen sich automatisiert Reports zu verschiedenen Themen erzeugen, bspw. Auswertungen je Land, Branche, Kunde etc.!
...
Ändert nun im Header des Dokuments unter "params" den Parameter `game_title` von "Fortnite" zu "League of Legends" und klickt erneut auf "knit". Was hat sich verändert?

#### Aufgabe 1.3

Verändert im Header des Dokuments des Parameter hinter "output" zu `word_document`. Was passiert?
...
Was ihr innerhalb der `lm()` (steht für _linear model_) Funktion seht, ist der spezielle Formel-Syntax von R. Die Tilde bedeutet dabei "wird regressiert auf", y ist in diesem Fall also unsere abhängige Variable, deren Wert durch x erklärt werden soll. Da der Fokus des Seminars nicht auf der Berechnung statistischer Modelle liegt (es sei denn, ihr wollt welche verwenden), werden wir vermutlich kaum mit Formeln in Berührung kommen. Für den Fall wisst ihr aber, wie sie funktionieren. Der `summary()` Befehl wird euch häufiger über den Weg laufen. Er tut unterschiedliche Dinge, je nachdem welchen _Typ_ der input hat. Im obigen Fall viel wird das objekt `regression` als geschätztes Modell erkannt und man erhält eine passende Auswertung. Wenn wir stattdessen einen Dataframe als argument geben erhalten wir deskriptive Statistiken für alle darin enthaltenen Variablen:
...
mean(twitch_data$view_count, na.rm = TRUE)#arithmetisches Mittel
min(twitch_data$view_count, na.rm = TRUE)#Minimum (kleinster Wert in der Auswahl)
max(twitch_data$view_count, na.rm = TRUE)#Maximum (größter Wert in der Auswahl)
sd(twitch_data$view_count, na.rm = TRUE)#Standardabweichung
...
describe_stats(twitch_data$viewer_count)

```

Falls das für euch wie Voodoo aussieht braucht ihr euch keine Sorgen machen. Der Fokus der Veranstaltung liegt nicht auf der Programmierung von Funktionen, sondern vor Allem in der Anwendung bereits bestehender. In den Fällen, in denen eigene Funktionen sinnvoll oder notwendig sind gebe ich euch passende.^[Was euch natürlich nicht davon abhalten soll, es selber zu versuchen!]
...
Das Dateiformat, mit dem ihr persönlich vermutlich am häufigsten gearbeitet habt, sind Excel-Workbooks (`.xlsx`). Falls euch Daten in dieser Form geliefert werden und ihr möchtet in R mit Ihnen arbeiten, habt ihr zwei Möglichkeiten. Entweder, ihr exportiert die Daten aus Excel im SV Format oder ihr benutzt ein externes Paket um mit Excel-Workbpoks zu interagieren, wie bspw. das `readxl` Paket. Wir konzentrieren uns im Rahmen des Seminars auf CSV Dateien.
...
#Aufgabe: "Gib mir die 10 Streamer des Spiels "Dota 2" mit den höchsten Zuschauerzahlen"

#mit basis R
twitch_data[grepl("Dota 2", twitch_data$game_title),][1:10,c(12, 6)]

#tidy
twitch_data %>% 
  filter(game_title == "Dota 2") %>% 
  select(login, viewer_count) %>%
  head(10)

```

Wie ihr seht kommen beide Varianten zum selben Ergebnis, die tidy-variante ist aber wesentlich übersichtlicher. Es gibt wahrscheinlich noch bessere Möglichkeiten mit Basis R zum selben Ergebnis zu kommmen, aber diese ist die, die mir eingefallen ist. 
