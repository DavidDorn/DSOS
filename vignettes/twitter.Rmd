---
title: "Scraping von Twitter Daten"
author: "David Dornekott"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Scraping von Twitter Daten}
...
```{r, eval = FALSE}
timelines <- get_timeline(c("elonmusk", "BillGates", "realDonaldTrump"), n = 3200)
```
...
Diese Funktion ist unser Brot und Butter, denn mit ihr lassen sich Tweets mit Schlagwoertern suchen, allerdings leider nur solche, die nicht älter als 6-9 Tage sind. Das erste Argument ist dabei `q`, für "query". Dieser ist immer ein String, der bis zu 500 Zeichen lang sein kann. Als zusaetzliche Argumente der Funktion lassen sich bspw. eine maximale Anzahl wiedergegebener tweets (**n**), eine geographische Einschraenkung auf einen bestimmten Bereich(geocode) und ob retweets gesucht werden sollen (**include_rts**) angeben. Weiterhin kann ich die Sprache der gesuchten Tweets einschraenken, was durchaus sinnvoll ist fuer die spaetere Auswertung. Ich kann bis zu 18,000 tweets je 15-Minuten Fenster sammeln. Wenn ich das argument `retyonratelimit = TRUE` setze, wird automatisch ein timeout gesetzt wenn dieses Limit überschritten wurde, sodass es danach automatisch weitergeht.

```{r, eval = FALSE}
tweets <- search_tweets("iphone 12 mini", include_rts = FALSE, n = 3000, lang = "en")
```

Leerzeichen werden dabei als "UND" interpretiert, d.h. es werden Tweets wiedergegeben, die alle Wörter **irgendwo** enthalten. Der obige Suchbegriff 
Wenn ich nur nach zusammenhängenden Wortkonstellationen suchen möchte, muss ich doppelte Anführungszeichen verwenden!
#### lookup_tweets()
...
Wir begnuegen uns erstmal damit, 1% der Tweets für 10 Sekunden zu streamen:
